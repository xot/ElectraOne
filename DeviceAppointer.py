...
class DeviceAppointer(ElectraOneBase):
    ...
    def __init__(self, c_instance):
        ...
        # register the track selection listener (will listen to device selections)
        view = self.song().view
    ...
    def _handle_selected_track_change(self):
        ...
        if track != prev_track:
            # remove device selection listener from previously selected track
            if prev_track:
                self.debug(5,f'Track { prev_track.name } deselected. Removing selected device listener')
            ...
            self.debug(5,f'Track { track.name } selected. Adding selected device listener')
            ...
            self.debug(5,f'Track { track.name } already selected. Ignoring.')
    ...
    def _handle_selected_device_change(self):
        ...
        if device:
            ...
            self.debug(5,f'Device { device_name } selected. Now appoint it.')
        else:
