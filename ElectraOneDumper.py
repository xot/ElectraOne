...
class ElectraOneDumper(io.StringIO, ElectraOneBase):
    ...
    def _append_json_float_fader(self, id, p, cc_info):
        """Append a float valued, untyped, fader. Adjust display resolution
        ...
        # formatter function to display them as floats. The integer fader
        # has 14 bits (CC) resolution, i.e. 16384 values. Fader resolution is
        # adjusted according to range
        try: # vmin/vmax may be too large to turn into a float (eg if "inf")
            (vmin,vmax) = _get_par_min_max(p)
            if ((vmax > 100) or (vmin < -100)) and ((vmax-vmin)*10 < 16384):
                self._append_json_generic_fader(cc_info, True, 10*vmin, 10*vmax,"formatLargeFloat")
            elif ((vmax > 10) or (vmin < -10)) and ((vmax-vmin)*100 < 16384):
                self._append_json_generic_fader(cc_info, True, 100*vmin, 100*vmax,"formatFloat")
            elif ((vmax > 1) or (vmin < -1)) and ((vmax-vmin)*1000 < 16384) :
                self._append_json_generic_fader(cc_info, True, 1000*vmin, 1000*vmax,"formatSmallFloat")
            elif ((vmax-vmin)*10000 < 16384):
                self._append_json_generic_fader(cc_info, True, 10000*vmin, 10000*vmax,"formatTinyFloat")
            else:
    ...
    def _append_json_fader(self, id, parameter, cc_info):
        ...
        if (vmin == None) or (vmax == None):
            ...
            # Ableton reports plugin value types incorrectly; assume float always 
