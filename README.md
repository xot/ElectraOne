...
The mixer preset is by default stored in bank 6, first slot. 
...
The Remote Script uploads a default mixer defined in ```consts.py```. This can be changed to suit your needs.
...
- Mon-quantised parameters are shown as faders on the Electra One. As many faders as possible are assigned to 14bit CCs. (These CCs actually occupy *two* slots in the CC map, see below.)
...
Note that the actual CC parameter used for a 14bit control is cc_no *and* cc_no+32 (because by convention a 14bit CC value is sent using the base CC and its 'shadow' parameter 32 higher. (This means the constructed map may appear to have holes in the 32-63 and 96-127 range.)
